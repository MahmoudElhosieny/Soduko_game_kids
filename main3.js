    ...
    for (let i = 0; i < inputArry.length; i++) {
        ...
        inputArry[i].addEventListener('keyup', function () {
            if (inputArry[i].value == '1') {
                picturePlace[inputArry[i].id].style.backgroundImage = `url(image3/${inputArry[i].value}.png)`;
...
function pictureSet(index, value) {
    ...
    if (inputArry[index].value == '1') {
        picturePlace[inputArry[index].id].style.backgroundImage = `url(image3/${inputArry[index].value}.png)`;
